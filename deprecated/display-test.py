...
def entry_split(entry, line_split=17, num_lines=7):
    ...
    for i in range(len(words)):
        if (
            (len(line_strings[line_index]) == line_split - 2 and len(words[i]) > 2)
...
def entry_display(entry, char_width=8, char_height=8, line_gap=4):
    ...
    line_count += int(
        ((inky_display.HEIGHT - 2 * char_height - 7) - ((char_height + line_gap) * line_count)) / char_height
    )
    lines_display(
        71 * sc,
        2 * char_height + 7,
        entry_split(gsc_format(entry), line_length, line_count),
        char_width,
        char_height,
        line_gap,
    )
...
entry_display(
    ...
    dex_config.getint("DEFAULT", "char_width"),
    dex_config.getint("DEFAULT", "char_height"),
)
